...
import { RotateCw, ThumbsUp, ThumbsDown, Keyboard, Layers, CheckCircle, XCircle, ArrowRight, List } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-end mb-4 px-2">
        <h2 className="text-xl font-bold text-gray-800">Kanji Index</h2>
        <span className="text-xs font-bold text-gray-400 uppercase">{KANJI_DATA.length} Characters</span>
      </div>

      <div className="space-y-3 pb-safe">
        {KANJI_DATA.map(kanji => (
          <div key={kanji.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex gap-4 hover:border-dojo-indigo transition-colors group">
            <div className="w-16 h-16 bg-slate-50 rounded-lg flex items-center justify-center border border-slate-100 group-hover:bg-indigo-50 group-hover:border-indigo-100 transition-colors">
              <span className="text-4xl font-black text-slate-800 group-hover:text-dojo-indigo">{kanji.kanji}</span>
            </div>

            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-1">
                <h3 className="font-bold text-gray-800 truncate">{kanji.meaning}</h3>
                <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded uppercase">{kanji.level}</span>
              </div>

              <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-xs">
                <span className="text-slate-400 font-bold uppercase">On</span>
                <span className="font-mono text-slate-600 truncate">{kanji.readings.onyomi.join(', ') || '-'}</span>

                <span className="text-slate-400 font-bold uppercase">Kun</span>
                <span className="font-mono text-slate-600 truncate">{kanji.readings.kunyomi.join(', ') || '-'}</span>
              </div>
            </div>
          </div>
        ))}
